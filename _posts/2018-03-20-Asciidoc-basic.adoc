...
문서를 빌드하면 html,pdf,epub등의 파일이 만들어지며 우리 프로젝트에서는 html 을 생성합니다.

asciidoc 은 문서 표준의 이름이고 이 문서를 빌드하기 위한 도구가 asciidoctor  입니다. asciidoctor 는 루비을 기반으로 만들어 졌기 때문에 루비를 설치 해야 합니다.
...
### 문단 (Paragraphs)
문단은 빈줄로 구분됩니다. 빈줄이 없이 쓰여진 문장은 하나의 문단으로 인식 되어 이어써집니다.
